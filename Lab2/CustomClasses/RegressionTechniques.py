...
class ClassificationTechniques(Exception):

    def __init__(self,domain_data,target_data,data_all,domain_data_test=[],target_data_test=[]):
        self.domain_data = domain_data
        self.target_data = target_data
        self.data_all = data_all
        self.domain_data_test = domain_data_test
...
class RegressionTechniques(Exception):
    ...
    def calculate_MSE(self,mean_axis=0):
        ...
        try:
            ...
            self.error_train = np.linalg.norm(self.yhat_train-self.y_train)**2
            ...
            self.error_test= np.linalg.norm(self.yhat_test-self.y_test)**2
        ...
        try:
            for i in range(loop_limit):
                ...
                if (i == 0):
                    ...
                    np.put (self.error_test, i, self.current_error_test)
                    np.put(self.w_vector,i,np.linalg.norm(self.w_hat))
                    np.put(self.cost_function_gradient_vector,i,self.gradient)

                elif (i > 0):
                    self.error_train = np.append(self.error_train, self.current_error_train)
                    self.error_test = np.append (self.error_test, self.current_error_test)
                    self.w_vector = np.append (self.w_vector, np.linalg.norm(self.w_hat))
                    self.cost_function_gradient_vector = np.append (self.cost_function_gradient_vector, self.gradient)
