...
class RegressionTechniques(Exception):
    ...
    def calculate_MSE(self,mean_axis=0):
        ...
        return self.yhat_train,self.error_train_diff,self.yhat_test,self.error_test_diff
            ...
            for i in range(loop_limit):
                ...
                if (i == 0):
                    ...
                    if (abs(self.w_vector[i-1] - self.w_vector[i]) <= stopping_condn or i >= loop_limit-1):
                        ...
                        self.logger.info ("Gradient Descent - Iteration count at stopping condn:" + str (i))
                        self.yhat_test = self.x_test.dot (self.w_hat)
                        self.yhat_train = self.x_train.dot (self.w_hat)
            ...
            for i in range(loop_limit):
                ...
                if (i == 0):
                    ...
                    if (abs(self.w_vector[i-1] - self.w_vector[i]) <= stopping_condn or i >= loop_limit-1):
                        ...
                        self.logger.info ("Steepest Descent - Iteration count at stopping condn:" + str (i))
                        self.yhat_test = self.x_test.dot (self.w_hat)
                        self.yhat_train = self.x_train.dot (self.w_hat)
