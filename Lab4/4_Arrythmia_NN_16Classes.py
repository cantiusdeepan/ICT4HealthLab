...
conf_matrix = tf.contrib.metrics.confusion_matrix (labels=tf.argmax (t, axis=1),
                                                   predictions=tf.argmax (tf.round (y), axis=1), num_classes=16)
...
sn.heatmap (conf_train, annot=True, annot_kws={"size": 12}, cmap="Blues", xticklabels=[i for i in range (1, 17)],
            yticklabels=[i for i in range (1, 17)])
plt.title ("Confusion matrix Neural Network 16 classes-Training")
plt.xlabel ("Class")
plt.ylabel ("Class")
plt.savefig ("Output/Final/" + "Confusion_matrix_nn_16-Training.png")
...
plt.figure ( )
sn.set (font_scale=1.4)  # for label size
sn.heatmap (conf_test, annot=True, annot_kws={"size": 12}, cmap="Blues", xticklabels=[i for i in range (1, 17)],
            yticklabels=[i for i in range (1, 17)])
plt.title ("Confusion matrix Neural Network 16 classes-Testing")
plt.xlabel ("Class")
plt.ylabel ("Class")
plt.savefig ("Output/Final/" + "Confusion_matrix_nn_16-Testing.png")
